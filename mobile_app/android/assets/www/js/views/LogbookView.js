...
window.LogBookView = Backbone.View
		.extend({
			...
			initialize : function() {
				...
				window.BetesApp.User.logEntries
						.bind('reset', this.addAll, this);
				window.BetesApp.User.logEntries.bind('remove', this.refresh,
						this);
			...
			showGraph : function(e) {
				if (e.target.hash == "#bs-graph") {
					this.showBloodSugarGraph(window.BetesApp.User.logEntries);
				} else if (e.target.hash == "#goals-graph") {
					this.showGoalsGraph(window.BetesApp.User.logEntries);
				} else if (e.target.hash == "#insights-graph") {
					this.showInsights(window.BetesApp.User.logEntries);
				}
			},
			...
			addAll : function(entries) {
				if (entries == null) {
					entries = window.BetesApp.User.logEntries.fetch();
				}
				if (entries != null) {
			...
			filterToday : function() {
				this.addAll(window.BetesApp.User.logEntries.filterToday());
			},
			filterYesterday : function() {
				this.addAll(window.BetesApp.User.logEntries.filterYesterday());
			},
			filterSeven : function() {
				this.addAll(window.BetesApp.User.logEntries.filterDays('7'));
			},
			filterThirty : function() {
				this.addAll(window.BetesApp.User.logEntries.filterDays('30'));
			},
			filterLogEntries : function(e) {
				var searchString = $(".filter-logbook").val();
				this.addAll(window.BetesApp.User.logEntries
